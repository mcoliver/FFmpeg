...
static int vk_prores_raw_start_frame(AVCodecContext *avctx,
  ...
  err = ff_vk_decode_prepare_frame_sdr(dec, prr->frame, vp, 1, FF_VK_REP_NATIVE,
                                       0);
  if (err < 0)
    return err;
...
static int vk_prores_raw_decode_slice(AVCodecContext *avctx,
  ...
  FFVkBuffer *slices_buf =
      vp->slices_buf ? (FFVkBuffer *)vp->slices_buf->data : NULL;
...
static int vk_prores_raw_end_frame(AVCodecContext *avctx) {
  ...
  ff_vk_frame_barrier(&ctx->s, exec, prr->frame, img_bar, &nb_img_bar,
                      ...
                      VK_ACCESS_2_TRANSFER_WRITE_BIT, VK_IMAGE_LAYOUT_GENERAL,
  ...
  ff_vk_shader_update_img_array(&ctx->s, exec, decode_shader, prr->frame,
                                vp->view.out, 0, 0, VK_IMAGE_LAYOUT_GENERAL,
                                VK_NULL_HANDLE);
  ff_vk_shader_update_desc_buffer(
      &ctx->s, exec, decode_shader, 0, 1, 0, frame_data_buf, 0,
      prr->nb_tiles * sizeof(TileData), VK_FORMAT_UNDEFINED);
  ...
  ff_vk_shader_update_img_array(&ctx->s, exec, idct_shader, prr->frame,
                                vp->view.out, 0, 0, VK_IMAGE_LAYOUT_GENERAL,
                                VK_NULL_HANDLE);
  ff_vk_shader_update_desc_buffer(
      &ctx->s, exec, idct_shader, 0, 1, 0, frame_data_buf, 0,
      prr->nb_tiles * sizeof(TileData), VK_FORMAT_UNDEFINED);
  ...
  for (int i = 0; i < 9; i++) {
    ...
    snprintf(val, sizeof(val), "%f", prr->ccm[i]);
    av_dict_set(&prr->frame->metadata, key, val, 0);
  }
...
static int init_decode_shader(AVCodecContext *avctx, FFVulkanContext *s,
  ...
  RET(ff_vk_shader_link(s, shd, ff_prores_raw_decode_comp_spv_data,
...
static int init_idct_shader(AVCodecContext *avctx, FFVulkanContext *s,
  ...
  RET(ff_vk_shader_link(s, shd, ff_prores_raw_idct_comp_spv_data,
...
static void vk_prores_raw_free_frame_priv(AVRefStructOpaque _hwctx,
                                          void *data) {
